            ...
            function go(){
              ...
              if(product_code_arr.includes(item.product_code)){
                ...
                product_code_arr.push(item.product_code)
                 connection.query("INSERT INTO `products`(`product_title_name`, `product_slug`, `store_name`, `product_description`, `product_type`, `brand`, `category`, `parent_category`, `seo_tag`, `other_introduction`, `add_custom_input`, `wholesale_sales_tax`, `manufacturers_sales_tax`, `retails_sales_tax`, `gst`, `value_added_tax`, `variety`, `vendor_id`, `shop`) VALUES ('" + item.product_title_name + "','" + item.product_slug + "','" + item.store_name + "','" + item.product_description + "','" + item.product_type + "','"+item.brand+"','" + item.category + "','" + item.parent_category + "','" + item.seo_tag + "','" + item.other_introduction + "','"+ add_custom_input1 + "','"+item.wholesale_sales_tax+"','"+item.manufacturers_sales_tax+"','"+item.retails_sales_tax+"','"+item.gst+"','"+item.value_added_tax+"',"+item.variety+",'"+item.vendor_id+"','"+item.shop+"')", (err, rows, fields) => {
                  if (err) {
                    console.log("/_products_post_error" + err)
                    //res.send(err)
                  } else {
