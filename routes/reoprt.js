...
return false
  connection.query("SELECT SUM(`sale_price`) gross_total_amount , SUM(`gst`) total_gst, SUM(DISTINCT `shipping_charges`)  total_shipping_charges FROM orders WHERE (`created_on` BETWEEN '" + req.body.from_date + " 00:00:00' AND '" + req.body.to_date + " 23:59:59') AND (NOT `status` = 'return') "+str_revenue+"", (err, rows, fields) => {
    if (err) {
      //console.log(err)
  ...
  connection.query("SELECT SUM(`sale_price`) return_total FROM `orders` WHERE `status` = 'return' AND (`created_on` BETWEEN '" + req.body.from_date + " 00:00:00' AND '" + req.body.to_date + " 23:59:59') "+str_revenue+"", (err, rslt, fields) => {
    if (err) {
      //console.log(err)
      res.status(500).send(err)
    } else {
  ...
  connection.query("SELECT SUM(coupons.percentage) as total_discount FROM coupons WHERE id IN(SELECT discount_coupon FROM orders WHERE (`created_on` BETWEEN '" + req.body.from_date + " 00:00:00' AND '" + req.body.to_date + " 23:59:59') AND (NOT `status` = 'return') "+str_revenue+")", (err, rows, fields) => {
    if (err) {
      //console.log(err)
      res.status(500).send(err)
    } else {
