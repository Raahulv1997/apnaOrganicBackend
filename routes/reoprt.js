...
console.log(req.body.prev_from_date) 
...
//(SELECT SUM(orders.shipping_charges) FROM orders WHERE orders.id=orders_view.order_id) as count
 ...
 connection.query("SELECT SUM(`sale_price`) gross_total_amount , SUM(`gst`) total_gst, (SELECT SUM(orders.shipping_charges) FROM orders WHERE (`created_on` BETWEEN '" + req.body.prev_from_date + " 00:00:00' AND '" + req.body.from_date + " 23:59:59')) as total_shipping_charges FROM orders_view WHERE (`created_on` BETWEEN '" + req.body.prev_from_date + " 00:00:00' AND '" + req.body.from_date + " 23:59:59') "+str_revenue+"", (err, result, fields) => {
 if (err) {
 console.log(err)
 res.status(200).send(err)
 } else {
 ...
 connection.query("SELECT SUM(`sale_price`) return_total FROM `orders_view` WHERE `status` = 'return' AND (`created_on` BETWEEN '" + req.body.prev_from_date + " 00:00:00' AND '" + req.body.from_date + " 23:59:59') " + str_revenue + "", (err, rslts, fields) => {
 if (err) {
 console.log(err)
 res.status(200).send(err)
 } else {
 ...
 connection.query('SELECT DISTINCT order_id,(SELECT SUM(orders.discount_coupon_value) FROM orders WHERE orders.id=orders_view.order_id) as count FROM `orders_view` WHERE status="delivered" AND discount_coupon!="" AND `created_on` BETWEEN "' + req.body.prev_from_date + ' 00:00:00" AND "' + req.body.from_date + ' 23:59:59" ' + str_revenue + '', (err, rows, fields) => {
 if (err) {
 console.log(err)
 res.status(200).send(err)
 } else {
...
function products_report(req, res) {
 ...
 connection.query('SELECT COUNT(DISTINCT `order_id`) as prev_order_count,SUM(`sale_price`) prev_net_sales,COUNT(product_id) as prev_product_count FROM orders_view WHERE status="delivered" AND `created_on` BETWEEN "' + req.body.prev_from_date + ' 00:00:00" AND "' + req.body.from_date + ' 23:59:59" AND NOT `status` = "return" ' + str_revenue + '', (err, rows, fields) => {
 if (err) {
 console.log(err)
 //res.status(200).send(err)
 } else {
...
function taxes_report(req, res) {
 ...
 connection.query('SELECT SUM(total_gst) prev_order_tax,COUNT(DISTINCT order_id) prev_order_count FROM orders_view WHERE `status`="delivered" AND `created_on` BETWEEN "' + req.body.prev_from_date + ' 00:00:00" AND "' + req.body.from_date + ' 23:59:59" ' + str_revenue + '', (err, rows, fields) => {
 if (err) {
 console.log(err)
 res.status(200).send(err)
 } else {
