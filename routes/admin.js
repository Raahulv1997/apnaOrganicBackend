...
async function admin_login(req, res) {
  ...
  connection.query(
    'SELECT `id`, `admin_email` , `admin_password` FROM `admin_login_details`  WHERE `admin_email` ="' +
      admin_email +
      '"',
    async (err, results) => {
      if (err) {
...
async function update_password(req, res) {
  ...
  if (admin_email != "" && admin_password != "" && new_admin_password != "") {
    ...
    connection.query(
      'SELECT `admin_email` , `admin_password` FROM `admin_login_details`  WHERE `admin_email` ="' +
        admin_email +
        '"',
      async (err, results) => {
        if (err) {
          ...
          if (results != "") {
            ...
            if (old_pswd == admin_password) {
              connection.query(
                'UPDATE `admin_login_details` SET `admin_password`= "' +
                  new_admin_password +
                  '" WHERE `admin_email` = "' +
                  admin_email +
                  '" AND `admin_password` = "' +
              ...
              res.send({ response: "password not matched" });
            }
          } else {
    ...
    async (err, results) => {
      if (err) {
        ...
        if (results != "") {
          ...
          var old_fr_psw = JSON.parse(
            JSON.stringify(results[0].admin_password)
          );
          // const salt = await bcrypt.genSalt(10);
            ...
            .sendMail(mail_configs, (err) => {
              if (err) {
                ...
                return res
                  .status(200)
                  .send({
                    message: "Sent your old password on Gmail Succesfully",
                  });
              }
            });
        } else {
...
function update_admin(req, res) {
  ...
  connection.query(
    'UPDATE `admin_login_details` SET `admin_name`="' +
      admin_name +
      '",`admin_phone`="' +
      admin_phone +
      '",`admin_type`="' +
...
async function add_admin(req, res) {
  //console.log("req.body")
  var { admin_email, admin_name, admin_phone, admin_type, admin_password } =
    req.body;
  // const salt = await bcrypt.genSalt(10);
  // co_salt = await bcrypt.hash(admin_password, salt);
  connection.query(
    'INSERT INTO `admin_login_details`(`admin_email`, `admin_name`, `admin_phone`, `admin_type`, `admin_password`) VALUES ("' +
      admin_email +
      '", "' +
      admin_name +
      '", "' +
...
function admin_search(req, res) {
  ...
  if (admin_name == "" && admin_type == "") {
    connection.query(
      "SELECT * FROM `admin_login_details` WHERE  1",
      (err, rows, fields) => {
        if (err) {
    ...
    if (admin_name != "" && admin_type == "") {
      connection.query(
        'SELECT * FROM `admin_login_details` WHERE `admin_name` LIKE "%' +
          admin_name +
          '%" ',
        (err, rows, fields) => {
          if (err) {
    ...
    if (admin_type != "" && admin_name == "") {
      connection.query(
        'SELECT * FROM `admin_login_details` WHERE `admin_type` LIKE "%' +
          admin_type +
          '%" ',
        (err, rows, fields) => {
          if (err) {
...
function admin(req, res) {
  if (req.query.id == "all") {
    ...
    connection.query(
      "SELECT * FROM admin_login_details WHERE id =" + req.query.id + " ",
      (err, rows, fields) => {
        if (err) {
...
function vendor_availability(req, res) {
  ...
  connection.query(
    'UPDATE `vendor` SET `availability`= "' +
      req.body.availability_change +
      '" WHERE `id` = ' +
      req.admin_vendor_com_id +
      "",
    (err, rows, fields) => {
      if (err) {
...
function vendor_requests(req, res) {
  connection.query(
    'SELECT * FROM vendor WHERE status ="pending" ',
    (err, rows, fields) => {
      if (err) {
...
function brand_list(req, res) {
  connection.query(
    "SELECT DISTINCT brand FROM products WHERE 1",
    (err, rows, fields) => {
      if (err) {
