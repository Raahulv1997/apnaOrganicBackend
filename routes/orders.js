                ...
                setTimeout(() => { 
                 ...
                 connection.query('SELECT `email` FROM users WHERE `user_id`='+user_id+'',(err, rslt) => {
                  if (err) {
                    ...
                    //SELECT * FROM `email_template` WHERE `type` = "user" AND `email_type` = "order_placed"
                    connection.query('SELECT * FROM `email_template` WHERE `type` = "user" AND `email_type` = "order_placed"',(err, rows) => {
                      if (err) {
                        ...
                        const mail_configs = {
                            ...
                            text: "your placed request confirm",
                            html: html_data
                        }
                        ...
                        nodemailer.createTransport({
                            ...
                            .sendMail(mail_configs, (err) => {
                                if (err) {
                                    ...
                                    return res.status(200).send({ "message": "Send confirm mail Succesfully", "order":"order_count_" + order_count + "" });
                                }
                            })
